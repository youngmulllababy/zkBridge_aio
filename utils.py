...
class Lifeform(Help):
    def __init__(self, privatekey, delay, api, proxy=None):
        ...
        self.proxy = f'http://{proxy}' if proxy else None
        self.moralisapi = api
        self.nft_address = Web3.to_checksum_address('0xf511832fe856D97e19Bdd81D3dba2300278910B9')
        self.nft = 'Lifeform x Polyhedra x Polygon Avatar'
        self.bridge_address = Web3.to_checksum_address('0x4da1b400a96cbb3d0f55c94af7bf7302c289dde8')
    ...
    async def bridge_nft(self, to_chain):
        ...
        async def bridge_():
            ...
            try:
                nonce = await self.w3.eth.get_transaction_count(self.address)
                args = self.nft_address, id_, chain_ids[to_chain], self.address, lz_adapt
                tx = await bridge.functions.transferNFT(*args).build_transaction({
                    'from': self.address,
                    'value': fee,
                    'nonce': nonce,
    ...
    async def do_lifeform_tasks(self):
        sign = await self.profile()
        if not sign:
            return self.privatekey, self.address, f'error while signing'
